...
async function weather(userinput) {
  ...
  try {
    ...
    // If the API returned an error (e.g., location not found), show an alert
    if (res.error) {
      alert(`${userinput}: is not a valid location name`);
      throw new Error(`API Error: ${res.error.message}`);
    }
    //inserting name of location
    locationdiv.childNodes[1].childNodes[1].childNodes[1].innerText =
      res.location.name;
    //inserting name of region
    locationdiv.childNodes[1].childNodes[1].childNodes[3].innerText =
      res.location.region;
    //insertin name of country
    locationdiv.childNodes[1].childNodes[1].childNodes[5].innerText =
    ...
    locationdiv.childNodes[3].childNodes[5].childNodes[1].innerText =
